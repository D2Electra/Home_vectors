...
class Line (object):
    ...
    def check_edges(self):
        ...
        if self.position.x < 0:
            self.velocity.x = -self.velocity.x
            self.position.x = 0
        if self.position.y > py5.height or self.position.y + 50 > py5.height or self.position.y - 50 > py5.height:
            self.velocity.y = -self.velocity.y
            self.position.y = py5.height - 50
        if self.position.y < 0 or self.position.y + 50 < 0 or self.position.y - 50 < 0:
            self.velocity.y = -self.velocity.y
            self.position.y = 50
